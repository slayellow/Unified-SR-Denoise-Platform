#!/usr/bin/env python3
"""Denoise probe: Deploy vs Restormer/NAFNet Teachers on 42 real images (memory efficient)."""
...
def main():
        ...
        with torch.no_grad():
            for idx, image_path in enumerate(images, 1):
                ...
                try:
                    ...
                    # Compute metrics
                    raw_metrics = basic_metrics(raw_bgr)
                    out_metrics = basic_metrics(output_bgr)

                    # Find or create row for this image
                    row = None
                    for r in rows:
                        if r["relative_path"] == str(rel_path):
                            row = r
                            break
    ...
    if rows:
        ...
        print(f"  Models: {len([m for m in MODELS if (PROJECT_ROOT / m).exists()])}")
